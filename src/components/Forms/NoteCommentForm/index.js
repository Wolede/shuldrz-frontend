...
const NoteCommentForm = props => {
  ...
  const validationSchema = Yup.object({
    comment: Yup.string()
      .max(
        1000,
        "You've hit the 1000 character limit. Add another comment after"
      )
      .required('Come on! Get it off your chest'),
  })

  const onSubmit = async values => {
    const req = prevComment
      ? api.put(`note-comments/${noteId}`, {
            ...
            <Form noValidate autoComplete="off">
              ...
              <FormControl className={classes.formControl}>
                <FormHelperText style={{ textAlign: 'center' }} error={true}>
                  {isSuccessful === false ? 'an error occured' : null}
                </FormHelperText>
              </FormControl>
            </Form>
          )}
