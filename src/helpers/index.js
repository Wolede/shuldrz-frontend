export const getProfileCompletion = user => {
  ...
  const profileKeys =
    user.userType === 'Volunteer' ? volunteerProfileKeys : guestProfileKeys
  ...
  const profilePercentage = `${Math.round(
    (profileScore / profileKeys.length) * 100
  )}%`
...
export const getGroupName = (mode, usersDetails, user) => {
  ...
  const membersMinusUser = members
    .filter((member, memberIndex) => member.userId !== user?.id)
    .map(item => item.username)
  const displayLimit = mode == 'chatList' ? 2 : 4

  const more =
    membersMinusUser.length - displayLimit > 0
      ? `+${membersMinusUser.length - displayLimit}`
      : ''
  ...
  const name =
    mode == 'chatList'
      ? membersMinusUser
          .filter((member, memberIndex) => memberIndex < displayLimit)
          .join(', ')
          .substring(0, truncateValue)
      : membersMinusUser
          .filter((member, memberIndex) => memberIndex < displayLimit)
          .join(', ')
